...
public class ExtremesMapper
    ...
    private boolean validTemp(double temperature) {
        if (temperature == NcdcConstants.EXTREME_TEMP_HIGH
                || temperature == NcdcConstants.EXTREME_TEMP_LOW) {
            return false;
        }
        return true;
    }
    ...
    protected void map(LongWritable key, Text value, Context context)
        ...
        try {
            ...
            if (validTemp(surfaceTemp)) {
                if (maxGroundTemp == Double.NEGATIVE_INFINITY || (surfaceTemp >= maxGroundTemp
                        && surfaceTemp - maxGroundTemp < threshold)) {
                    checkGroundTemp = true;
                    maxGroundTemp = surfaceTemp;
                }
                if (minGroundTemp == Double.MAX_VALUE || (surfaceTemp <= minGroundTemp
                        && minGroundTemp - surfaceTemp < threshold)) {
                    checkGroundTemp = true;
            ...
            if (checkAirTemp || checkGroundTemp) {
                ...
                TemperatureWritable temperatureWritable = new TemperatureWritable(checkAirTemp
                        ? airTemp : NcdcConstants.EXTREME_TEMP_HIGH,
                                                                                  checkGroundTemp
                                                                                          ? surfaceTemp
